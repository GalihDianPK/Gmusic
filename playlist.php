...
$p_query = "SELECT * FROM playlists WHERE id = $playlist_id AND user_id = $user_id";
$p_result = mysqli_query($conn, $p_query);
if(mysqli_num_rows($p_result) == 0) {
    echo "<div class='text-white p-6'>Playlist not found or access denied.</div>";
    require 'footer.php';
    exit();
}
$playlist = mysqli_fetch_assoc($p_result);

// Remove specific song from playlist
if(isset($_GET['remove_song'])) {
    $song_id = intval($_GET['remove_song']);
    mysqli_query($conn, "DELETE FROM playlist_songs WHERE playlist_id = $playlist_id AND song_id = $song_id LIMIT 1");
    // Redirect cleanly
    echo "<script>window.location.href='playlist.php?id=$playlist_id';</script>";
    exit();
}

// Fetch songs in this playlist
$songs_query = "
    SELECT songs.*, playlist_songs.added_at 
    FROM songs 
...
<div class="h-full flex flex-col relative w-full pt-6">
    <!-- Header Banner -->
    <div class="flex flex-col md:flex-row items-center md:items-end gap-6 p-6 pb-8 border-b border-white/10 shrink-0 bg-gradient-to-b from-[#404040] to-transparent">
        <div class="w-48 h-48 md:w-56 md:h-56 shadow-2xl shrink-0 bg-card-dark rounded-md flex items-center justify-center">
            <span class="material-symbols-outlined text-8xl text-slate-500">music_note</span>
        </div>
    ...
    <div class="px-6 pb-24 flex-1">
        <?php if (count($tracks_array) > 0): ?>
            <script>
                const currentPlaylistQueue = <?php echo json_encode($tracks_array); ?>;
                function playPlaylist() {
                    if (typeof window.playQueue === 'function') {
                        window.playQueue(currentPlaylistQueue, 0);
                    }
                }
            </script>
            ...
            <?php foreach($tracks_array as $index => $song): ?>
                <div class="group grid grid-cols-12 items-center hover:bg-white/10 p-2 rounded-md transition-colors cursor-pointer"
                     onclick="window.playQueue(currentPlaylistQueue, <?php echo $index; ?>)">
                    ...
                    <div class="col-span-5 md:col-span-3 flex items-center justify-end gap-4 opacity-0 group-hover:opacity-100 pr-2">
                        <a href="playlist.php?id=<?php echo $playlist_id; ?>&remove_song=<?php echo $song['id']; ?>" class="text-slate-400 hover:text-red-500 transition-colors" title="Remove from playlist" onclick="event.stopPropagation(); return confirm('Remove this song?');">
        ...
        <?php else: ?>
            <div class="flex flex-col items-center justify-center text-slate-400 pt-12">
                <span class="material-symbols-outlined text-6xl mb-4">music_note</span>
                <p class="text-lg">Let's find some songs for your playlist</p>
                <a href="search.php" class="mt-4 px-6 py-2 bg-white text-black font-bold rounded-full hover:scale-105 transition-transform">Find songs</a>
            </div>
        <?php endif; ?>
    </div>
</div>
